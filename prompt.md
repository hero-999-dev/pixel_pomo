...
- `lib/logic.dart` — pure port of every Kotlin pure class (engine, themes, flowers, economy,
  garden, labels, label colors, stats, test data), **plus** `Placeables` (road/fence objects,
  5 coins) and `Garden.connectionMask` (auto-tiling bitmask of same-id neighbours, no edge wrap).
- `lib/strings.dart` (6 languages + month names) · `lib/store.dart` (`AppStore` ChangeNotifier
  ...
  `lib/pixel.dart` (pixel widgets + bar/line/pie chart painters) · `lib/main.dart` (all screens).
- **Custom garden engine** `lib/engine/garden_engine.dart` + `garden_view.dart`: a small
  purpose-built 2.5D renderer (no Flame/Unity). An oblique **look-from-above** projection you
  **tilt** (slider), **pinch-zoom** and **pan**; a **gapless green grass field** + soil slab for
  depth; flowers that stand up & sway; a flock of **randomly-wandering pixel bugs**. No tile
  numbers shown; tap-to-place keeps an exact screen↔tile inverse.
- **Art as data:** every drawable object is a crisp **PNG in `assets/objects/`** (grass, bug,
  road, fence + 10 flowers), emitted by the dependency-free `tools/gen_objects.py`.
...
- `test/logic_test.dart` (pure parity + placeables/no-cap/connectionMask) +
...
- CI: **`.github/workflows/build-flutter.yml`** runs on a **macOS runner**, scaffolds the
  ...
  --no-codesign`, zips the unsigned `.ipa`, and publishes both to the **`latest-flutter`**
  prerelease. The `.ipa` is **sideloaded with SideStore/AltStore** (signs on-device, no Mac).
