...
APK and an **unsigned iOS `.ipa`**. It's a faithful port of the same v0.5.0 behavior — do
**not** add features here that the native app doesn't have; keep them at parity.

- `lib/logic.dart` — pure port of every Kotlin pure class (engine, themes, flowers, economy,
  garden, labels, label colors, stats, test data).
- `lib/strings.dart` (6 languages + month names) · `lib/store.dart` (`AppStore` ChangeNotifier
  + `shared_preferences` + wall-clock countdown) · `lib/pixel.dart` (pixel widgets + bar/line/
  pie chart painters) · `lib/main.dart` (all screens/overlays).
- `test/logic_test.dart` (pure parity tests) + `test/widget_smoke_test.dart` (boots the app,
  opens every overlay, asserts no exceptions/overflow). Both gate CI.
- **Only the portable parts are committed** (`lib/`, `pubspec.yaml`, `assets/`, `test/`).
